...
func GetUserNotificationSettings(userID int) (models.NotificationSettings, error) {
	...
	err := database.DB.QueryRow(ctx, `
		SELECT id, user_id, telegram_notification, discord_notification, vk_notification, created_at, updated_at
...
func UpdateUserNotificationSettings(userID int, telegram, discord, vk bool) (models.NotificationSettings, error) {
	ctx := context.Background()
	...
	err = database.DB.QueryRow(ctx, `
		...
		SET telegram_notification = $1, discord_notification = $2, vk_notification = $3, updated_at = NOW()
		WHERE user_id = $4
		RETURNING telegram_notification, discord_notification, vk_notification, updated_at`,
		telegram, discord, vk, userID).Scan(
		&settings.TelegramNotification,
		&settings.DiscordNotification,
		&settings.VKNotification,
		&settings.UpdatedAt,
